...
class UnitConverter:
    ...
    @staticmethod
    def normalize_sampling_interval_to_rate(sampling_interval: Union[float, int]) -> int:
        """
        ...
        for group_name, group_config in power_groups.items():
            ...
            if "sampling_interval" in group_config:
                ...
                if not isinstance(sampling_interval, (int, float)) or sampling_interval <= 0:
                    raise ValueError(f"Invalid sampling_interval for {group_name}: must be positive number")
                
                # Convert to rate and remove sampling_interval
                rate = UnitConverter.normalize_sampling_interval_to_rate(sampling_interval)
                group_config["rate"] = rate
                del group_config["sampling_interval"]
                logger.info(f"Converted {group_name} sampling_interval {sampling_interval}s to rate {rate}Hz")
            
            # Validate rate parameter
            if "rate" in group_config:
                rate = group_config["rate"]
                if not isinstance(rate, int) or rate <= 0:
                    raise ValueError(f"Invalid rate for {group_name}: must be positive integer (Hz)")
                group_config["rate"] = rate
