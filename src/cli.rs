use crate::config::{EmtConfig, MeasurementUnitsConfig};
use crate::metrics_sink::{MetricsSink, PrometheusSink, SharedPrometheusSink, prometheus_router};
use crate::monitor::{DeviceEnergy, MetricsSnapshot, Monitor, MonitorHandle};
...
mod tests {
    ...
    #[test]
    fn cli_run_from_returns_success_for_help() {
        assert_eq!(run_from(["emt", "--help"]), 0);
    }

    #[test]
    fn cli_run_from_returns_error_for_invalid_mode_args() {
        assert_eq!(run_from(["emt", "--duration", "1"]), 2);
    }
}
